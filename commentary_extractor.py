...
class CommentaryExtractor:
    ...
    def __extract_commentary__(self, url):
        ...
        commentary_events = [{'over': self.__extract_over_num__(div.find('div', attrs={'class': 'odi_commentary-overs'})),
                              'text': self.__extract_commentary_text__(div.find('div', attrs={'class': 'odi_commentary-text'}))} for div in commentary_event_divs]
...
if __name__ == '__main__':
    ...
    for url in all_odis_url:
        ...
        if comm_data == "[]":
            print("No odi_commentary data in: %s" % url)
        else:
